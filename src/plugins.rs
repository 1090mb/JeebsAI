...
impl Plugin for LogicPlugin {
	...
	async fn handle(&self, input: &str, _state: &crate::state::AppState) -> Option<String> {
		let lower = input.to_lowercase();
		if lower.contains("logic") || lower.contains("and") || lower.contains("or") || lower.contains("not") {
			let expr = lower.replace("logic", "").replace("?", "").trim().to_string();
			if expr.is_empty() {
...
impl Plugin for SystemPlugin {
	...
	async fn handle(&self, _input: &str, state: &crate::state::AppState) -> Option<String> {
		...
		Some(format!("System Info: {:.1}% CPU usage, {} MB used / {} MB total RAM", cpu_usage, used_mem / 1024, total_mem / 1024))
	}
}
use std::sync::Mutex;
...
impl Plugin for MemoryPlugin {
	...
	async fn handle(&self, input: &str, _state: &crate::state::AppState) -> Option<String> {
		...
		} else if lower.contains("recall") || lower.contains("what did you remember") || lower.contains("what do you remember") {
...
impl Plugin for CalcPlugin {
	...
	async fn handle(&self, input: &str, _state: &crate::state::AppState) -> Option<String> {
		let lower = input.to_lowercase();
		if lower.contains("calculate") || lower.contains("what is") || lower.contains("solve") || lower.contains("math") {
			let expr = lower.replace("calculate", "").replace("what is", "").replace("solve", "").replace("math", "").replace("?", "").trim().to_string();
			if expr.is_empty() {
...
impl Plugin for TimePlugin {
	...
	async fn handle(&self, input: &str, _state: &crate::state::AppState) -> Option<String> {
		...
		if lower.contains("time") || lower.contains("date") || lower.contains("clock") {
			let now = Local::now();
			Some(format!("The current time is {}.", now.format("%Y-%m-%d %H:%M:%S")))
		} else {
			None
		}
	}
