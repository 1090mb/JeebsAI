...
function renderUpdates() {
    ...
    pageItems.forEach(function(u) {
        ...
        }).join("") || '<div class="text-muted" style="font-style:italic;font-size:0.85em">No comments yet.</div>';
        ...
        if (rootAdmin && u.status === "pending") {
            actions += '<div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap"><button class="btn btn-accent" onclick="applyUpdate(\'' + u.id + '\')">Apply</button><button class="btn btn-danger" onclick="denyUpdate(\'' + u.id + '\')">Deny</button><button class="btn" onclick="resolveUpdate(\'' + u.id + '\')">Resolve</button></div>';
        } else if (rootAdmin && u.status === "applied") {
            actions += '<div style="margin-top:6px"><button class="btn btn-danger" onclick="rollbackUpdate(\'' + u.id + '\')">Rollback</button></div>';
        }
    ...
    if (pages > 1) {
        ...
        var label = document.createElement("div"); label.className = "text-muted"; label.textContent = "Page " + currentPage + " / " + pages;
